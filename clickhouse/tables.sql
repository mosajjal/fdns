CREATE TABLE IF NOT EXISTS DNS_LOG (
  ...
  TTL toDate(PacketTime) + INTERVAL 30 DAY DELETE -- DNS_TTL_VARIABLE
  ;
...
CREATE TABLE IF NOT EXISTS DNS_DOMAIN_COUNT (
  ...
  ORDER BY (Server, QH)
  SAMPLE BY QH
  TTL DnsDate + INTERVAL 30 DAY DELETE -- DNS_TTL_VARIABLE
  ;
...
CREATE TABLE IF NOT EXISTS DNS_DOMAIN_UNIQUE (
  ...
  TTL toDate(timestamp)  + INTERVAL 30 DAY DELETE -- DNS_TTL_VARIABLE
  ;
